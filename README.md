...
- Run ```pip install irc requests bs4``` to install the libraries needed to run this bot. You may need to create a virtualenv with ```python3 -m venv webirc-venv; source webirc-venv/bin/activate``` on UNIX like systems or ```py -m venv webirc-venv; webirc-venv\Scripts\activate```
